...
-- Fix get_study_stats_summary: explicit text cast to avoid date=text type mismatch
CREATE OR REPLACE FUNCTION public.get_study_stats_summary(p_user_id uuid, p_tz_offset_minutes integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- FIX: explicit cast both sides to text to avoid date=text operator error
  v_daily_energy_earned := CASE WHEN COALESCE(v_profile.last_study_reset_date, '')::text = v_today::text THEN COALESCE(v_profile.daily_energy_earned, 0) ELSE 0 END;
  v_today_cards := CASE WHEN COALESCE(v_profile.last_study_reset_date, '')::text = v_today::text THEN COALESCE(v_profile.daily_cards_studied, 0) ELSE 0 END;
...
-- Create consolidated community deck updates RPC
CREATE OR REPLACE FUNCTION public.get_community_deck_updates(p_user_id uuid)
 RETURNS TABLE(local_deck_id uuid, has_update boolean)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  RETURN QUERY
  WITH user_community_decks AS (
    SELECT d.id AS local_id, d.source_turma_deck_id, d.synced_at
    FROM decks d
    WHERE d.user_id = p_user_id
      AND d.source_turma_deck_id IS NOT NULL
  ...
  source_info AS (
    SELECT
      ucd.local_id,
      ucd.synced_at,
      td.deck_id AS source_deck_id
    FROM user_community_decks ucd
    JOIN turma_decks td ON td.id = ucd.source_turma_deck_id
  ),
  latest_mod AS (
    SELECT
      si.local_id,
      ...
      GREATEST(
        sd.updated_at,
        (SELECT MAX(c.updated_at) FROM cards c WHERE c.deck_id = si.source_deck_id)
      ) AS last_modified
    FROM source_info si
    JOIN decks sd ON sd.id = si.source_deck_id
  )
  SELECT
    lm.local_id AS local_deck_id,
    (lm.synced_at IS NULL OR lm.last_modified > lm.synced_at) AS has_update
  FROM latest_mod lm
  WHERE lm.last_modified IS NOT NULL;
END;
$function$;
