...
-- Fix count_descendant_cards_by_state to use last_rating instead of difficulty ranges
...
AS $function$
  ...
  counts AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE c.state = 0 AND c.last_rating IS NULL) AS diff_novo,
      COUNT(*) FILTER (WHERE c.last_rating = 4) AS diff_facil,
      COUNT(*) FILTER (WHERE c.last_rating = 3) AS diff_bom,
      COUNT(*) FILTER (WHERE c.last_rating = 2) AS diff_dificil,
      COUNT(*) FILTER (WHERE c.last_rating = 1) AS diff_errei
    FROM cards c
