-- Fix: RPCs don't handle NULL scheduled_date for new cards
-- but the study queue query does, causing count mismatch.

-- 1) Fix count_descendant_cards_by_state
...
SELECT
  c.deck_id,
  COUNT(*) FILTER (WHERE (c.state = 0 OR c.state IS NULL) AND (c.scheduled_date IS NULL OR c.scheduled_date <= ft.threshold)),
...
SELECT
  COUNT(*) FILTER (WHERE (dc.state = 0 OR dc.state IS NULL) AND (dc.scheduled_date IS NULL OR dc.scheduled_date <= ft.threshold)),
...
AS $function$
  SELECT
    COUNT(*) FILTER (WHERE c.state = 0 AND (c.scheduled_date IS NULL OR c.scheduled_date <= now() + interval '50 years')) AS total_new,
    COUNT(*) FILTER (WHERE c.state = 2 AND c.scheduled_date <= now() AND c.scheduled_date <= now() + interval '50 years') AS total_review,
    COUNT(*) FILTER (WHERE c.state IN (1, 3) AND (c.scheduled_date IS NULL OR c.scheduled_date <= now() + interval '50 years')) AS total_learning
  FROM cards c
