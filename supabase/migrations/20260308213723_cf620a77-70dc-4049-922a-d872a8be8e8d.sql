...
BEGIN
  ...
  -- Safe: cast the column to text first, then compare. Avoid COALESCE with empty string on date columns.
  v_reset_date_text := v_profile.last_study_reset_date::text;
  v_daily_energy_earned := CASE WHEN v_reset_date_text IS NOT NULL AND v_reset_date_text = v_today::text THEN COALESCE(v_profile.daily_energy_earned, 0) ELSE 0 END;
  v_today_cards := CASE WHEN v_reset_date_text IS NOT NULL AND v_reset_date_text = v_today::text THEN COALESCE(v_profile.daily_cards_studied, 0) ELSE 0 END;
