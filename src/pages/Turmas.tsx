/**
 * Explorar — grid of public decks with search and tag filters.
 */
...
import { usePublicDecks } from '@/hooks/useTurmas';
      ...
      <header className="sticky top-0 z-10 border-b border-border/40 bg-background/80 backdrop-blur-sm">
        <div className="container mx-auto flex items-center gap-3 px-4 py-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">Descubra decks públicos</p>
          </div>
        </div>
      </header>
            ...
            {filteredDecks.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl py-16 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1 max-w-xs">
                  {searchQuery ? `Nenhum resultado para "${searchQuery}"` : 'Nenhum deck público disponível.'}
                </p>
              </div>
            )}
          </>
