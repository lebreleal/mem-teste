...
const Study = () => {
  ...
  useEffect(() => {
    if (queue.length > 0 && !queueInitialized) {
      setLocalQueue([...queue]); setInitialQueueSize(queue.length); setQueueInitialized(true);
      sessionStartRef.current = Date.now();
      const statsMap = new Map<string, DeckSessionStats>();
      for (const card of queue) {
        const did = card.deck_id as string;
        if (!statsMap.has(did)) {
          const cfg = deckConfigs[did]; const name = cfg?.name || (deckConfig?.name) || 'Baralho';
          statsMap.set(did, { deckId: did, deckName: name, total: 0, done: 0, correct: 0, wrong: 0 });
        }
        statsMap.get(did)!.total += 1;
      }
      deckStatsRef.current = statsMap; setDeckStatsSnapshot(Array.from(statsMap.values()));
    }
  ...
  const { data: sourceInfo } = useQuery({
    ...
    queryFn: () => resolveCommunitySource(currentCardDeckId!),
    enabled: !!currentCardDeckId, staleTime: 5 * 60_000,
  });

  useEffect(() => {
    const now = Date.now();
    const futureTimes = localQueue.filter(c => c.state === 1 || c.state === 3).map(c => new Date(c.scheduled_date).getTime()).filter(t => t > now);
    if (futureTimes.length === 0) return;
    const delay = Math.min(...futureTimes) - now + 100;
  ...
  useEffect(() => {
    ...
    const now = Date.now(); const soonest = Math.min(...localQueue.map(c => new Date(c.scheduled_date).getTime()));
    const remaining = Math.max(0, Math.ceil((soonest - now) / 1000)); setWaitingSeconds(remaining);
    if (remaining <= 0) { setLearningTick(prev => prev + 1); return; }
    const interval = setInterval(() => { const r = Math.max(0, Math.ceil((soonest - Date.now()) / 1000)); setWaitingSeconds(r); if (r <= 0) { clearInterval(interval); setLearningTick(prev => prev + 1); } }, 1000);
    return () => clearInterval(interval);
  ...
  const executeReview = useCallback((card: StudyCard, rating: Rating) => {
    ...
    if (shouldKeep) {
      ...
      setLocalQueue(prev => { const idx = prev.findIndex(c => c.id === card.id); if (idx < 0) return prev; const updatedCard = { ...prev[idx], state: estimatedState, scheduled_date: estimatedDate, learning_step: stepIdx }; const without = [...prev.slice(0, idx), ...prev.slice(idx + 1)]; return [...without, updatedCard]; });
    } else {
      const buriedSiblingIds: string[] = [];
      setLocalQueue(prev => {
        let filtered = prev.filter(c => c.id !== card.id);
        if (card.card_type === 'cloze') {
          const cardCfg = getCardDeckConfig(card); const buryNew = cardCfg?.bury_new_siblings !== false; const buryReview = cardCfg?.bury_review_siblings !== false; const buryLearning = cardCfg?.bury_learning_siblings !== false;
          if (buryNew || buryReview || buryLearning) {
            const siblingIds = getSiblingIds(card, filtered);
            if (siblingIds.length > 0) { filtered = filtered.filter(c => { if (!siblingIds.includes(c.id)) return true; if (c.state === 0 && buryNew) { buriedSiblingIds.push(c.id); return false; } if (c.state === 2 && buryReview) { buriedSiblingIds.push(c.id); return false; } if ((c.state === 1 || c.state === 3) && buryLearning) { buriedSiblingIds.push(c.id); return false; } return true; }); }
          }
        }
        return filtered;
      });
      if (buriedSiblingIds.length > 0) { const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate() + 1); tomorrow.setHours(0, 0, 0, 0); buryCards(buriedSiblingIds, tomorrow.toISOString()); }
    }
    setIsTransitioning(true);
    setTimeout(() => { setCardKey(prev => prev + 1); cardShownAt.current = Date.now(); submittingRef.current = null; setIsTransitioning(false); }, 200);
  ...
  if (!currentCard && !allWaiting && reviewCount > 0) {
    if (!sessionDoneRef.current) { sessionDoneRef.current = true; setTimeout(() => goBack(), 1500); }
    return <SessionComplete reviewCount={reviewCount} />;
  }

  if (allWaiting) {
    const mins = Math.floor(waitingSeconds / 60); const secs = waitingSeconds % 60;
    const timeStr = mins > 0 ? `${mins}:${secs.toString().padStart(2, '0')}` : `${secs}s`;
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4">
        <div className="animate-fade-in text-center">
          <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-primary/10"><Brain className="h-10 w-10 text-primary animate-pulse" /></div>
          <h1 className="font-display text-2xl font-bold text-foreground">Aguardando repetição</h1>
          <p className="mt-2 text-muted-foreground">{localQueue.length} {localQueue.length === 1 ? 'card está' : 'cards estão'} em aprendizado.</p>
          ...
          <Button variant="outline" onClick={goBack} className="mt-8 gap-2"><ArrowLeft className="h-4 w-4" /> Voltar</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4">
        <div className="animate-fade-in text-center">
          ...
          <Button variant="outline" onClick={goBack} className="mt-6 gap-2"><ArrowLeft className="h-4 w-4" /> Voltar</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-[100dvh] flex-col bg-background overflow-hidden">
      <header className="sticky top-0 z-20 bg-background flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3">
        <div className="flex items-center gap-1.5">
          <Button variant="ghost" size="icon" onClick={goBack} className="h-8 w-8 text-muted-foreground"><ArrowLeft className="h-4 w-4" /></Button>
        </div>
      ...
      <main ref={mainScrollRef} className="flex flex-1 min-h-0 items-center justify-center px-2 sm:px-4 py-2 sm:py-4 overflow-y-auto">
        <div key={cardKey} className={`w-full transition-all duration-200 ${isTransitioning ? 'opacity-0 scale-95' : 'animate-fade-in'}`}>
          <FlashCard
            cardId={currentCard.id} frontContent={currentCard.front_content} backContent={currentCard.back_content}
            ...
            cardType={currentCard.card_type} learningStep={currentCard.learning_step ?? 0}
            ...
            isTutorLoading={tutor.isTutorLoading} hintResponse={tutor.hintResponse}
            explainResponse={tutor.explainResponse} mcExplainResponse={tutor.mcExplainResponse}
            canUndo={undo.canUndo} onUndo={undo.handleUndo}
            ...
            communityMeta={sourceInfo ? (
              <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground mt-1">
                {sourceInfo.authorName && <span>por <span className="font-medium text-foreground">{sourceInfo.authorName}</span></span>}
                {sourceInfo.updatedAt && <span className="flex items-center gap-0.5"><RefreshCw className="h-2.5 w-2.5" />{formatDistanceToNow(new Date(sourceInfo.updatedAt), { addSuffix: true, locale: ptBR })}</span>}
                <button onClick={() => setCommunityInfoOpen(true)} className="flex items-center justify-center rounded-full text-muted-foreground hover:text-foreground transition-colors" aria-label="Informações do card de comunidade"><Info className="h-3 w-3" /></button>
              </div>
            ) : undefined}
          />
        </div>
      ...
      <StudyDialogs
        ...
        chatCardContext={currentCard ? { front: currentCard.front_content, back: currentCard.back_content } : undefined}
        explainInChat={explainInChat} activeStreamingResponse={activeStreamingResponse}
