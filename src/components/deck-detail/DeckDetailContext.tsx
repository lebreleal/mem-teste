    ...
    (async () => {
      try {
        ...
        if (!sourceDeckId && isLiveDeck) {
          const { data: candidates } = await supabase.from('decks').select('id').eq('name', (deck as any).name).eq('is_public', true).neq('user_id', user.id).limit(1);
        ...
        while (hasMore) {
          ...
          await supabase.from('cards').insert(cards.map((c: any) => ({ deck_id: deckId, front_content: c.front_content, back_content: c.back_content, card_type: c.card_type ?? 'basic', state: 0, stability: 0, difficulty: 0 })) as any);
