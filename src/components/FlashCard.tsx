...
const FlashCard = ({
  ...
  energy = 0, tutorCost = 2, onTutorRequest, isTutorLoading, hintResponse, explainResponse, mcExplainResponse, actions, actionsLeft, communityMeta,
  ...
  useEffect(() => {
    if (!isOcclusion) { setOcclusionFallbackCanvas(null); return; }
    try {
    ...
    } catch { setOcclusionFallbackCanvas(null); }
  ...
  if (isOcclusion) {
    ...
    if (backContent && backContent.trim()) {
      try {
        const parsed = JSON.parse(backContent);
        if (typeof parsed.clozeTarget === 'number') { clozeTarget = parsed.clozeTarget; extraBack = parsed.extra || ''; }
        else extraBack = backContent;
      } catch { extraBack = backContent; }
    }
    ...
    <div className="flex flex-col w-full max-w-lg mx-auto px-1 h-[calc(100dvh-7rem)] relative">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide p-4 -m-4">
        <div className="space-y-3 pb-2 pt-1 w-full">
          ...
          >
            ...
            {flipped && (
              <>
                ...
                >
                  <button
                    onClick={(e) => { e.stopPropagation(); setPeekingFront(prev => !prev); }}
                    className={`absolute top-2.5 right-2.5 flex h-7 w-7 items-center justify-center rounded-full transition-all ${
                      peekingFront ? 'text-primary bg-primary/10' : 'text-muted-foreground/50 hover:text-muted-foreground hover:bg-muted/60'
                    }`}
          ...
          {/* Tutor explain response (fallback) */}
            ...
            {typingAnswer ? (
              <div className="flex w-full gap-2 items-end">
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' && typedAnswer.trim()) { setAnswerSubmitted(true); setFlipped(true); }
                    else if (e.key === 'Escape') { setTypingAnswer(false); setTypedAnswer(''); }
                  }}
                ...
                <button
                  onClick={() => { if (typedAnswer.trim()) { setAnswerSubmitted(true); setFlipped(true); } }}
